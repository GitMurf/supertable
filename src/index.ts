...
const hideColumns = (t: HTMLDivElement, visibilityConfig: VisibilityConfig) => {
  ...
  rows.forEach((tr) => {
    ...
    cells.forEach((td, index) => {
      td.hidden = !visibilityByIndex[index];
    });
  });
};
...
const setupTable = (tableElement: HTMLDivElement) => {
  ...
  ths.forEach((th, index) => {
    ...
    // field we're talking about. Note that the first column will be "---".
    th.dataset.attribute = th.innerText;
  ...
  // "Default Sort:: rating=Asc, title=Desc" -> ["rating=Asc", "title=Desc"]
  const defaultSort = (config["Default Sort"]
    ?.split(",")
    ?.map((s: string) => s.trim()) || []) as string[];

  defaultSort.forEach((s: string, i: number) => {
    const parts = s.split("=").map((s: string) => s.trim()); // ["rating", "Asc"]
    const field = parts[0];
    const direction = parts[1];
    sortConfig[field].priority = i + 1;
    sortConfig[field].asc = direction.toUpperCase() === "ASC";
  });
