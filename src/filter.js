...
export function filterSetup(roamTableElem, filterConfig) {
  var tableHeaderRow = roamTableElem.getElementsByTagName("tr")[0];
  ...
  filterRowCt.style.cssText = 'display:flex;color:#666;font-size:0.55em;margin-left:15px;margin-top:4px;'
  ...
  for(var col = 0; col < tableHeaderRow.cells.length; col++)
  {
      ...
      if(curFilterVal == '')
      {
        ...
        for (var y = 0; y < getChildBlocks.length; y++)
        {
          ...
          if(foundColTitle > -1)
          {
            ...
            if(colTitle.toLowerCase() == currentColName.toLowerCase())
            {
    ...
    if (filterConfig && filterIcon.dataset.filterName == filterConfig.field) {
      filterIcon.dataset.filterValue = filterConfig.filter;
      filterIcon.click();
    }
  }
  ...
  newInput.oninput = function () {
    ...
    for(var i = 1; i < tableRows.length; i++) //Skip header row starting with 1 instead of 0
    {
      ...
      for (var itm = 0; itm < filterArr.length; itm++)
      {
  ...
  if(filterConfig){newInput.dispatchEvent(new Event('input'));}
};
